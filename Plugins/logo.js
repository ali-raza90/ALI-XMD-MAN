...
zokou({ nomCom: "hacker",
    ...
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        const noArgMsg = `*_EXEMPLE *:  ${prefixe}dragonball ALI`;
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto ALI");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t*ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
           repondre(`*exemple :* ${prefixe}didong ALI`)
            return ;
        }
...
zokou({nomCom: "summer", categorie: "Logo", reaction: "🌞"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-sunset-light-text-effects-online-for-free-1124.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ' }, { quoted: ms });
    })
    .catch(console.error);
});
  ...
  {nomCom:"boomlg",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom ALI`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*"})
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent ALI-XMD");return;
      }
...
zokou({ nomCom: "gif×1", categorie: "Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*ʟᴏɢᴏ ᴍᴀᴅᴇ ʙʏ ᴀʟɪ-xᴍᴅ*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
